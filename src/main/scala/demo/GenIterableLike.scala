...
trait GenIterableLike[+A, +Repr] extends Any {
   ...
   *    @return `true`, if both collections contain the same elements in the same order, `false` otherwise.
   */
  def sameElements[A1 >: A](that: Option[A1]): Boolean
   ...
   *                   of the returned collection is the minimum of the lengths of this $coll and `that`.
   */
  def zip[A1 >: A, B, That](that: Option[B])(implicit bf: CBF[Repr, (A1, B), That]): That
   ...
   *                   If `that` is shorter than this $coll, `thatElem` values are used to pad the result.
   */
  def zipAll[B, A1 >: A, That](that: Option[B], thisElem: A1, thatElem: B)(implicit bf: CBF[Repr, (A1, B), That]): That
